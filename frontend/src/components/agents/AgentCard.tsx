    ...
    <div className="rounded-xl border border-border bg-card p-4 hover:border-primary/50 transition-colors">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          <div
            className={cn("w-2.5 h-2.5 rounded-full", (statusColors[agent.status] || statusColors.idle).dot)}
            aria-hidden="true"
          />
          <h3 className="font-semibold text-sm">{agent.name}</h3>
          <span className="sr-only">{(statusColors[agent.status] || statusColors.idle).label}</span>
        </div>
